...
<%
            ...
            JSONObject objSkipedDocInfo = new JSONObject();
            try{
            JSONObject skipDocsInfo = (JSONObject) request.getAttribute("SKIPPED_DOCS_INFO");
            if(skipDocsInfo != null){
            objSkipedDocInfo = skipDocsInfo;
              AppLogger.debug("request Skipped Docs  page::"+objSkipedDocInfo); 
            }
            }catch(Exception e){
...
<script>
    var llSkipedDocInfo ='<%=objSkipedDocInfo%>';
      var doc_file_path=""; 
       var reqBackToImg=false;
       if(llSkipedDocInfo != 'null'){
         llSkipedDocInfo= JSON.parse(llSkipedDocInfo);  
       }
    </script>
      <script >   
...
<body onload="setFormDD();setDocsInfo()">
      ...
      <div id="content-wrapper">
        ...
        <div class="row box mrg25B" id="page_content_block" >
          ...
          <div class="alert alert-info">
            <p><s:text name="dekyc.Digital.Uploadnote"/></p>
          </div>
          <div class="row">
              <div>
              <div class="col-md-4">
...
<!--                  <button type="button" class="btn btn-default" id="regi2">Take a photo</button>
            ...
            <div class="ticklist">
              <ul>
                ...
                <li><s:text name="dekyc.Digital.Fileli2"/> </li>
...
<!--                <div class="file_upload btn btn-default">
          ...
          <div class="clear mrg20B"></div>
          <div class="form-group col-md-12 pad20L"> <a onclick="uploadSkippedDocs('LLDKYC');" class="primarybt" title="Update">Upload Documents</a> 
              <!--<a  onclick="nxtToDKYCLLForm('LL_SKIP_ATTACH');" class="primarybt1" title="Back">Skip</a>-->
...
//                                    reqData.imagePath = attachmentsFilePath;
   ...
   function captureImage() {
            ...
            function setDocsInfo(){
                var ispoipoasame=false;
                 $("#poi_type option:contains("+llSkipedDocInfo.POI_TYPE+")").prop('selected', true);
                 $("#poi_type").trigger('change');
                 $("#poi_type").prop('disabled',true);
                if(llSkipedDocInfo.POI_TYPE == llSkipedDocInfo.POA_TYPE){
                    ispoipoasame= true;
                $('#poiSamePoa').hide();
                }else{
                $("#poa_type option:contains("+llSkipedDocInfo.POA_TYPE+")").prop('selected', true);
                $('#poa_type').trigger('change');
                $("#poa_type").prop('disabled',true);
                $("#checkPOA").prop('disabled',true);
                }
            }
</script>

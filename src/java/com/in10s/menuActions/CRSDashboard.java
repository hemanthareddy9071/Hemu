...
public class CRSDashboard {
    ...
    public String fetchCurdayStatus() {
        AppLogger.info(" Starting fetchCurdayStatus grid load method ");
        ...
        try {
            ...
            String CUR_DAY_STATUS_QRY = new CRSPropertyReader().getQueryonId("CUR_DAY_STATUS_QRY").replace("@@DBSCHEMA@@", CRSAppResources.DB_SCHEMA);
            pstmt = con.prepareStatement(CUR_DAY_STATUS_QRY);
            pstmt.setString(1, loginResponse.getString("UserCode"));
            pstmt.setString(2, loginResponse.getString("UserCode"));
            AppLogger.debug("fetchCurdayStatus quary is:::" + CUR_DAY_STATUS_QRY);
            rs = pstmt.executeQuery();
            while (rs.next()) {
                CurdayJOBJ = new JSONObject();
                CurdayJOBJ.put("TOT_CNT", rs.getString("TOT_CNT"));
                CurdayJOBJ.put("SUCCESS_CNT", rs.getString("SUCCESS_CNT"));
                CurdayJOBJ.put("REJ_CNT", rs.getString("REJ_CNT"));
                CurdayJOBJ.put("PEN_CNT", rs.getString("PEN_CNT"));
            ...
            AppLogger.debug("CurdayARR ::: " + CurdayARR);
            ...
            AppLogger.error("Exception in fetchCurdayStatus grid load method : " + sw.toString());
        } finally {
            if (rs != null) {
                try {
                    rs.close();
        ...
        AppLogger.info(" Ending of fetchCurdayStatus grid load method ");
        return retValue;
    }

    public String fetchBarGraphValues() {
        AppLogger.info(" Starting fetchBarGraphValues grid load method ");
        String retValue = "success";
        Connection con = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        ...
        try {
            ...
            String BAR_GRAPH_QRY = new CRSPropertyReader().getQueryonId("BAR_GRAPH_QRY").replace("@@DBSCHEMA@@", CRSAppResources.DB_SCHEMA);
            pstmt = con.prepareStatement(BAR_GRAPH_QRY);
            pstmt.setString(1, reqJson.getString("minDate"));
            pstmt.setString(2, reqJson.getString("maxDate"));
            pstmt.setString(3, loginResponse.getString("UserCode"));
            pstmt.setString(4, loginResponse.getString("UserCode"));
            AppLogger.debug("fetchBarGraphValues quary is:::" + BAR_GRAPH_QRY);
            rs = pstmt.executeQuery();
            ...
            ex.printStackTrace(pw);
            response.setSuccess(false);
            AppLogger.error("Exception in fetchBarGraphValues grid load method : " + sw.toString());
        } finally {
            if (rs != null) {
                try {
                    rs.close();
        ...
        AppLogger.info(" Ending of fetchBarGraphValues grid load method ");
        return retValue;
    }

    public String fetchPieGraphValues() {
        AppLogger.info(" Starting fetchPieGraphValues grid load method ");
        String retValue = "success";
        Connection con = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        ...
        try {
            ...
            String PIE_GRAPH_QRY = new CRSPropertyReader().getQueryonId("PIE_GRAPH_QRY").replace("@@DBSCHEMA@@", CRSAppResources.DB_SCHEMA);
            pstmt = con.prepareStatement(PIE_GRAPH_QRY);
            pstmt.setString(1, reqJson.getString("minDate"));
            pstmt.setString(2, reqJson.getString("maxDate"));
            pstmt.setString(3, loginResponse.getString("UserCode"));
            pstmt.setString(4, loginResponse.getString("UserCode"));
            AppLogger.debug("fetchPieGraphValues quary is:::" + PIE_GRAPH_QRY);
            rs = pstmt.executeQuery();
            ...
            try {
                for (int i = 1; i <= rs.getFetchSize(); i++) {
                    name = rsmd.getColumnName(i);
                    System.out.println(":::substring::::::::::::::::::::::" + StringUtils.substringBefore(name, "_"));
                    if (name.equalsIgnoreCase("CYMN") || name.equalsIgnoreCase("MNP")) {
                        headerList.put(i + "", name);
                    } else {
            ...
            while (rs.next()) {
                for (int i = 1; i <= headerList.size(); i++) {
                    ...
                    if (dataValues == null) {
                        ...
                        if (headerList.getString(i + "").equalsIgnoreCase("Normal")) {
                            jOBJ1.put("color", "#46a5da");
                        } else if (headerList.getString(i + "").equalsIgnoreCase("MNP")) {
                            jOBJ1.put("color", "#a8c374");
                        } else if (headerList.getString(i + "").equalsIgnoreCase("Fancy")) {
                            jOBJ1.put("color", "#ffc539");
                        } else if (headerList.getString(i + "").equalsIgnoreCase("CYMN")) {
            ...
            AppLogger.debug("PieGraphARR ::: " + PieGraphARR);
            response.setSuccess(true);
            ...
            ex.printStackTrace(pw);
            response.setSuccess(false);
            AppLogger.error("Exception in fetchPieGraphValues grid load method : " + sw.toString());
        } finally {
            if (rs != null) {
                try {
                    rs.close();
        ...
        AppLogger.info(" Ending of fetchPieGraphValues grid load method ");
        return retValue;
    }

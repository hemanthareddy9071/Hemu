...
<body onload="setCustomerData();loadPlanAmount();">
    <form name="CDRtrialPaymentPage" enctype="multipart/form-data">
        ...
        <div class="col-xs-12 pad0A col-md-8">
            <div class="box1 pad15A">
                ...
                <div class="infoList1">
                    ...
                    <div  id="plan_dtls_div" style="display: none">
                        <div class="row">
                            <div class="col-md-6 form-group" id="name_plan">
                                ...
                                <input  id="plan_org_name"  type="text" class="form-control"  onchange="validatePOIName(this);"  value="" minlength="3" maxlength="50">
                            </div>
                    ...
                    <div class="clear mrg10B"></div>
                    <div class="col-md-12">
                        <button id='take_pymt_btn' onclick="payReqforCDRRenewal();" class="primarybt">Pay</button>
                        <button id='disconnect_btn' onclick="disconnectTrialPlan();" style="display: none" class="primarybt">Disconnect</button>
                        <a  id='backButton'   href='Login.do' class="secondarybt" >Cancel</a>
                    </div>
                    <div>  
                    </div>
                </div>
...
<script>   
        ...
        function payReqforCDRRenewal(){  
          ...
          if(selTarifPlan != 0 && selTarifPlan != null ){
                ...
                if (selTarifPlan == "WTPO3" || selTarifPlan == "WTPO1" ) {             
                    ...
                    if (!validateCRDUserFile(userFile, alrtplanatch , 'input_POI')) {
                        $('#wait').hide();
                        return false;
                    }
          ...
          reqData.FILE_PATH=imgFilePath;
          
           $.ajax({
            url: "takePaymentForCDRRenewal.do", //parameters go here in object literal form
            type: 'POST',
            async: false,
        ...
        function validateCRDUserFile(userFile,dispName,id){
            ...
            alert("Upload size should be less than 3 KB");
            $("#" + id).val("");
            return false;
            }
...
<script>
        ...
        function validTrialTarifAttach(obj) {
            ...
            var fileName = userFile.value;
            validateCRDUserFile(userFile,alrtplanatch,id);
            var formData = new FormData();
            formData.append("userFile", $(obj).prop('files')[0]);
            uploadCDRImage(fileName, formData);
        }
        function uploadCDRImage(strId, formData) {
            var reqData = {};
            reqData.imageName = 'TARIFF_ID';
            reqData.imgFlodLoc = imgFilePath;
            reqData.CAF_NO = JSON.parse(custPayReqData).CAF_NO;
            formData.append("reqData", JSON.stringify(reqData));
            $.ajax({
                url: "uploadCDRImage.do",
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
                success: function (response) {
                    ...
                    imgFilePath=response.response.message;
                },
        ...
        function disconnectTrialPlan(){
          ...
          document.CDRtrialPaymentPage.method = "post";
          document.CDRtrialPaymentPage.action = "disconnectCDRTrial.do";
          document.CDRtrialPaymentPage.reqData.value = encrypt(JSON.stringify(reqData));
          document.CDRtrialPaymentPage.submit();
       }
